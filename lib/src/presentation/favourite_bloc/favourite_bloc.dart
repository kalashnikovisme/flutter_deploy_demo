...
class FavoritesBloc extends Bloc<FavoritesEvent, FavoritesState> {
  ...
  void _onLoadFavsList(FavoritesLoadEvent event, Emitter<FavoritesState> emit) async {
    final albumsList = await service.getFavoriteCharacters(userEmail);
    emit(FavoritesState(favoriteItems: albumsList ?? [], isFavourite: true, isLoading: false, email: userEmail));
  }

  void _onAddToFavorites(AddToFavoritesEvent event, Emitter<FavoritesState> emit) async {
  ...
  void _onRemoveFromFavorites(RemoveFromFavoritesEvent event, Emitter<FavoritesState> emit) async {
    final updatedFavorites = List.of(state.favoriteItems);
