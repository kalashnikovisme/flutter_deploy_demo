...
class PagBloc extends Bloc<PagEvent, PagState> {
  final ApiService apiService = ApiService();

  PagBloc() : super(const PagState(page: 1, count: 20, result: [], isLoading: false)) {
  ...
  void _onLoadData(LoadListEvent event, Emitter<PagState> emit) async {
    ...
    emit(state.copyWith(result: newData.results, page: nextPage));
  }

  void _onLoadNextPage(LoadNextPageEvent event, Emitter<PagState> emit) async {
    final nextPage = state.page + 1;
    final newData = await apiService.getResult(nextPage, state.count);
    final updatedData = List.of(state.result)..addAll(newData.results?.toList() ?? []);
    emit(
      state.copyWith(
        result: updatedData,
        page: nextPage,
